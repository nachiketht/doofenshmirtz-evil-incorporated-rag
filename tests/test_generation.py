...
def test_generate_answer_uses_provided_llm() -> None:
    ...
    assert generate_answer("Do you need a joke in email?", [_hit()], llm=_Stub()) == "Yes."


def test_build_generation_response_maps_chunks_and_router() -> None:
    payload = build_generation_response(" You can. ", [_hit()], RouteDecision(lane="current", source="llm"))
...
def test_generation_schema_rejects_extra_and_too_many_chunks() -> None:
    ...
    with pytest.raises(ValidationError):
        GenerationResponse.model_validate(
            {"answer": "ok", "retrieved_chunks": [chunk], "router": "current (regex)", "extra": 1}
        )
    with pytest.raises(ValidationError):
        GenerationResponse.model_validate(
            {"answer": "ok", "retrieved_chunks": [chunk] * 6, "router": "current (regex)"}
        )
...
def test_stale_excerpt_is_labeled_superseded() -> None:
    ...
    stale = _hit(
        text="Header\nold rule body",
        metadata={"policy_id": "p", "version": "1.0", "section_path": "4.2", "change_status": "stale"},
    )
...
def test_build_generation_response_caps_at_five_chunks() -> None:
    hits = [_hit(id=f"c{i}", metadata={"policy_id": "p", "version": "2.0", "section_path": str(i)}) for i in range(8)]
